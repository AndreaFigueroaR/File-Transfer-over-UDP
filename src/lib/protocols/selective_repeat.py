...
class SelectiveRepeat(ProtocolARQ):
    def __init__(self, socket, remote_address, is_verbose):
        super().__init__(socket, remote_address, is_verbose)
        self.pkt_id = 0
    ...
    def _send_segment_sr(self, segment_sn, pkt_id, data_chunk: bytes):
        ...
        segment = seq_num_byte + pkt_id_byte + data_chunk
        self.socket.sendto(segment, self.address)

    def _recv_ack_sr(self):
        segment, _ = self.socket.recvfrom(3)
        ...
        while attempts < NUM_ATTEMPS:
            try:
                ...
                if seq_num < expected_sn:
                    ...
                    # Store consecutive segments that were in the buffer and move forward
                    expected_sn = seq_num + 1
                    while (expected_sn, self.pkt_id) in segments_buffer:
                        data.extend(segments_buffer.pop((expected_sn, self.pkt_id)))
    ...
    def _send_ack_sr(self, ack, pkt_id):
        ...
        self.socket.sendto(ack_b + pkt_id_b, self.address)
