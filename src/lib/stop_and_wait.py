...
class StopAndWait(ProtocolARQ):
    ...
    def send(self, data: bytes):
        ...
        for i in range(0, len(data), TAM_BUFFER):
            segment = data[i:i+TAM_BUFFER] # Verificar que no rompe si no se cumple con el TAM_BUFFER
            self._send_segment(segment)
            
    #aqui se divide en tamaños fijos maximo
    
    #TODO : hacer parte del header de cada segmento el largo del payload (aparte del numero de secuencia)
