...
class ServerRDT: 
    ...
    def meet_client(self, client_data, server_prot_type) -> str:
        ...
        self.client_num_seq, client_prot_type, app_data = handshaker.handshake(client_data, self.peer)#podría alnzar una excepción
        if client_prot_type != server_prot_type:
            raise ValueError(f"Tipo de protocolo inválido: {client_prot_type}. Expected {server_prot_type}")
        self.protocol = self._init_protocol()
        return app_data
    ...
    def _init_protocol(self, prot_type):
        if prot_type == PROT_SR: 
            return SelectiveRepeat(self.peer)
        return StopAndWait(self.peer)
