...
class ClientHandshaker:
    ...
    def handshake(self, client_type, client_prot_type, file_path, skt):
        srv_num_seq = self._send_first_handshake_msg(
            skt, client_type, client_prot_type, file_path)
        self._send_second_handshake_msg(skt, srv_num_seq)
        return srv_num_seq

    def _send_first_handshake_msg(
            self, skt, client_type, client_prot_type, file_path):
    ...
    def _formatted_client_info(
        ...
        return f"{self.num_seq}|{client_prot_type}|{file_path}|{client_type}"
